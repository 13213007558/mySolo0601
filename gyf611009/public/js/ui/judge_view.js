...
function scoreInputs(ws, maxScore) {
    const inputs = {};
    const rows = [];
...
export function renderJudgeView(root, state) {
    clear(root);
    ...
    const statusBadge = (() => {
        ...
        const label = currentState.status === 'active' ? '品鉴中' : currentState.status === 'locked' ? '已锁定' : '待品鉴';
    ...
    const { container: scoreSection, getValues } = scoreInputs(currentState, state.config.maxScorePerDimension);
    ...
    unlockBtn.addEventListener('click', () => {
        if (!judge || !currentBlindId)
            return;
        store.requestUnlock(judge.id, currentBlindId, `评委${judge.name}申请解锁酒款 ${currentBlindId}`, () => { });
    });
    ...
    const listTitle = h('h3', { style: 'font-size:16px;margin:32px 0 16px;color:var(--text-secondary);font-weight:600' }, [t('全部酒款进度')]);
    ...
    const wineList = h('div', { class: 'wine-list' }, wineCards.filter(Boolean));
    root.appendChild(header);
    root.appendChild(judgeSelector);
...
//# sourceMappingURL=judge_view.js.map