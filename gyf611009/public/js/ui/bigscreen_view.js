...
export function renderBigscreenView(root, state) {
    clear(root);
    ...
    for (const judge of state.judges.values()) {
        ...
        const statusText = ws?.status === 'locked'
            ? '已锁定'
            : ws?.status === 'active'
                ? '品鉴中'
                : '待品鉴';
    ...
    const footer = h('div', {
        style: 'margin-top:48px;padding-top:24px;border-top:1px solid var(--border);text-align:center;color:var(--text-muted);font-size:14px;letter-spacing:0.1em',
    }, [
        t('本大屏为只读监督模式 · 评委每口品鉴后必须点击吐酒计数 · 达上限自动锁定 · 解锁须监审 PIN 留痕'),
    ]);
    const wrapper = h('div', { class: 'bigscreen' }, [header, grid, footer]);
    root.appendChild(wrapper);
}
//# sourceMappingURL=bigscreen_view.js.map