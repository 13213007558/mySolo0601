...
const PIN_LENGTH = 4;
export function renderPinModal(root, state) {
    ...
    const svOptions = state.config.supervisors.map((s) => h('option', { value: s.id, selected: s.id === supervisorId }, [t(s.name)]));
    const svSelect = h('select', { class: 'select', style: 'width:100%;margin-bottom:16px' }, svOptions);
...
//# sourceMappingURL=pin_modal.js.map