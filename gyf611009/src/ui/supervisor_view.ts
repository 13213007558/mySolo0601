...
export function renderSupervisorView(root: HTMLElement, state: AppState): void {
  clear(root);
  ...
  for (const judge of state.judges.values()) {
    for (const ws of judge.wines.values()) {
      ...
      if (ws.status === 'locked') totalLocked++;
      if (ws.totalScore !== null) totalScored++;
      if (ws.unlockedBySupervisor) unlockCount++;
    }
  }
  ...
  const title2 = h(
    'h3',
    { style: 'font-size:16px;margin:8px 0 16px;color:var(--text-secondary);font-weight:600' },
    [t('实时操作日志（最近 100 条）')],
  );
  ...
  const rows = reversed.map((entry) => {
    const actionCls =
      entry.action === 'SUPERVISOR_UNLOCK' || entry.action === 'SIP_LIMIT_EXCEEDED'
        ? 'unlock'
  ...
  const judgeOptions = Array.from(state.judges.values()).map((j) =>
    h('option', { value: j.id }, [t(`${j.seatNumber}号 · ${j.name}`)]),
  );
  const wineOptions = state.config.wines.map((w) =>
    h('option', { value: w.blindId }, [t(`${w.blindId} · ${w.realName} (${w.realVintage})`)]),
  );

  const judgeSel = h('select', { class: 'select', style: 'flex:1' }, judgeOptions) as HTMLSelectElement;
  const wineSel = h('select', { class: 'select', style: 'flex:1' }, wineOptions) as HTMLSelectElement;
  ...
  const row = h(
    ...
    [judgeSel, wineSel, quickUnlockBtn],
  );

  // Wine mapping table
  const mappingTitle = h(
    'h3',
    { style: 'font-size:16px;margin:32px 0 16px;color:var(--text-secondary);font-weight:600' },
    [t('盲品序号 ↔ 真实酒款映射（后台可见）')],
  );
  ...
  const mapRows = state.config.wines.map((w) =>
    h('div', { class: 'audit-log-row' }, [
