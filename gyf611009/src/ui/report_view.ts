...
import type { AppState, JudgeWineState } from '../types.js';
import { DIMENSION_LABELS, SCORE_DIMENSIONS } from '../types.js';

export function renderReportView(root: HTMLElement, state: AppState): void {
  clear(root);
  ...
  for (const wine of state.config.wines) {
    const ws: JudgeWineState | undefined = judge.wines.get(wine.blindId);
    if (!ws) continue;
