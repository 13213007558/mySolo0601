...
export function createApp(root: HTMLElement): () => void {
  ...
  function renderSidebar(state: AppState): void {
    clear(sidebar);
    ...
    for (const item of NAV_ITEMS) {
      const btn = h(
        'button',
        { class: `nav-btn${state.currentView === item.id ? ' active' : ''}` },
        [
    ...
    const rules = h(
      ...
      {
        style:
          'padding:0 20px;color:var(--text-muted);font-size:12px;line-height:1.7',
      },
      [
