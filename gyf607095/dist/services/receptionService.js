...
exports.receptionService = exports.ReceptionService = void 0;
const database_1 = require("../models/database");
const utils_1 = require("../utils");
const permissionService_1 = require("./permissionService");
class ReceptionService {
    initSteps() {
        if (database_1.db.receptionSteps.size > 0)
            return;
        const steps = [
            {
        ...
        for (const step of steps) {
            database_1.db.receptionSteps.set(step.id, step);
        }
    }
    getSteps(user) {
        permissionService_1.permissionService.assertPermission(user, 'step', 'view');
        return Array.from(database_1.db.receptionSteps.values())
            .sort((a, b) => a.order - b.order);
    }
    getStepByOrder(user, order) {
        permissionService_1.permissionService.assertPermission(user, 'step', 'view');
        return Array.from(database_1.db.receptionSteps.values())
            .find(s => s.order === order);
    }
    validateStepCompletion(user, order, data) {
...
exports.receptionService = new ReceptionService();
//# sourceMappingURL=receptionService.js.map