...
separator();
console.log('\n【第一步】主管刘主管登录，导入样例数据');
app_1.system.login('liuzhuguan');
separator();
console.log('\n【第二步】导入样例数据（第一次导入）');
app_1.system.importSampleData(sampleData_1.SAMPLE_DATA_FILE);
sleep(100);
separator();
console.log('\n【第三步】切换前台张前台登录');
app_1.system.login('zhangqiantai');
separator();
console.log('\n【第四步】查看前台操作步骤（新人照做）');
app_1.system.showReceptionSteps();
separator();
console.log('\n【第五步】查看手工补录前后差异');
app_1.system.showManualEntryComparison();
separator();
console.log('\n【第六步】前台批量核销处理');
app_1.system.processWriteOff();
separator();
console.log('\n【第七步】查看小明的详情（包含反馈）');
const xiaomingRecord = app_1.system.findChildByName('小明');
if (xiaomingRecord) {
    app_1.system.getChildDetail(xiaomingRecord.id);
}
separator();
console.log('\n【第八步】查看前台视图（只看必要内容）');
const receptionView = app_1.system.getReceptionView();
console.log('\n 前台汇总：');
...
separator();
console.log('\n【第十步】查看主管视图（完整审计）');
const supervisorView = app_1.system.getSupervisorView();
console.log('\n 主管汇总（与前台口径一致）：');
...
const allCourses = app_1.system.getAllCourses();
supervisorView.exceptionDetails.forEach(r => {
    const child = app_1.system.getAllChildren().find(c => c.id === r.childId);
...
supervisorView.manualEntryRecords.forEach(r => {
    const child = app_1.system.getAllChildren().find(c => c.id === r.childId);
    const course = allCourses.find(c => c.id === r.courseId);
    console.log('  ' + (child?.name || '未知') + ' - ' + (course?.name || '未知') + ' | 扣款 ' + (0, utils_1.formatCurrency)(r.amount));
});
separator();
console.log('\n【第十一步】验证两边视图口径一致');
app_1.system.compareViews();
separator();
console.log('\n【第十二步】治疗师李治疗师更新反馈');
app_1.system.login('lizhiliaoshi');
const xiaomingSchedule = app_1.system.findScheduleByChildAndTherapist('小明', 'therapist_001');
if (xiaomingSchedule) {
    app_1.system.updateFeedback(xiaomingSchedule.id, '今天小明表现特别好！平衡木训练连续走了3次不摇晃，专注力提升明显。继续保持！');
}
separator();
console.log('\n【第十三步】验证反馈版本一致性（结算和详情读同一版本）');
app_1.system.login('zhangqiantai');
if (xiaomingSchedule && xiaomingRecord) {
    const settlementFeedback = feedbackService_1.feedbackService.getFeedbackForSettlement(xiaomingSchedule.id);
    console.log('\n 前台结算读取的反馈：');
    console.log('  版本: v' + settlementFeedback.version);
    console.log('  内容: ' + settlementFeedback.feedback);
    console.log('\n 孩子详情读取的反馈：');
    const detailFeedback = feedbackService_1.feedbackService.getFeedback(xiaomingSchedule.id);
    console.log('  版本: v' + detailFeedback.version);
    console.log('  内容: ' + detailFeedback.feedback);
    const consistency = feedbackService_1.feedbackService.verifyFeedbackConsistency(xiaomingSchedule.id, settlementFeedback.version || 0);
...
if (exceptionRecords.length > 0) {
    ...
    app_1.system.auditRecord(firstException.id, '已核实照片确实缺失，同意按部分成功处理，已通知治疗师下次注意。');
}
separator();
console.log('\n【第十六步】验证余额安全（避免月底变负）');
const xiaogangRecord = app_1.system.findChildByName('小刚');
if (xiaogangRecord) {
    console.log('\n 小刚当前余额: ' + (0, utils_1.formatCurrency)(xiaogangRecord.balance));
