...
import {
  DisinfectionRecord, RecordStatus, UserRole, ApiResponse, ScanRecord
} from '../shared/types.js';

const router = Router();

function getRole(req: Request): UserRole {
  const roleHeader = (req.headers['x-user-role'] as string) || 'consultant';
  const validRoles: UserRole[] = ['consultant', 'store_staff', 'supervisor', 'admin'];
  return validRoles.includes(roleHeader as UserRole) ? (roleHeader as UserRole) : 'consultant';
}

function getOperatorInfo(req: Request) {
  const role = getRole(req);
  const id = (req.headers['x-user-id'] as string) || 'u_consultant_1';
  const name = (req.headers['x-user-name'] as string) || '李老师';
...
router.get('/me', (req, res) => {
  ...
  res.json({ success: true, data: op } as ApiResponse);
});
...
router.get('/records', (req, res) => {
  ...
  if (babyId) list = list.filter(r => r.babyId === babyId);
...
router.post('/records/:id/handle', (req, res) => {
  const op = getOperatorInfo(req);
  const body: HandleAbnormalBody = req.body;
  const record = db.records.find(r => r.id === req.params.id);
  if (!record) {
    return res.status(404).json({ success: false, error: '记录不存在' } as ApiResponse);
  }
  if (!body.status) {
    return res.status(400).json({ success: false, error: '缺少 status 参数' } as ApiResponse);
  }
...
router.post('/records/:id/revert', (req, res) => {
  ...
  if (op.role !== 'supervisor' && op.role !== 'admin') {
    return res.status(403).json({ success: false, error: '只有主管或管理员可以回退状态' } as ApiResponse);
  }
  if (record.status !== 'processed') {
    return res.status(400).json({ success: false, error: '只能回退已处理的记录' } as ApiResponse);
  }

  const oldStatus = record.status;
  const newStatus: RecordStatus = 'reverted';
  ...
  db.addAudit(
    ...
    '状态回退（保留审计供主管复查）',
    record.id, oldStatus, newStatus,
    { revertReason: { old: undefined, new: (req.body as any)?.reason || '主管复查' } },
    getClientIp(req)
  );
...
router.post('/records/:id/remarks', (req, res) => {
  ...
  if (!content) {
    return res.status(400).json({ success: false, error: '备注内容不能为空' } as ApiResponse);
  }
  ...
  res.json({ success: true, data: remark } as ApiResponse);
});

interface BatchHandleBody {
  items: { id: string; status: RecordStatus; handleResult: string; supplementRemark?: string }[];
}

router.post('/records/batch-handle', (req, res) => {
  const op = getOperatorInfo(req);
  const body: BatchHandleBody = req.body;
  if (!body.items || !Array.isArray(body.items)) {
    return res.status(400).json({ success: false, error: 'items 参数无效' } as ApiResponse);
  }
  ...
  for (const item of body.items) {
    try {
      ...
      if (!record) { failed.push({ id: item.id, reason: '记录不存在' }); continue; }
      const validTransitions: Record<RecordStatus, RecordStatus[]> = {
      ...
      record.processorName = op.name;
      record.processTime = Date.now();
      if (item.handleResult) record.handleResult = item.handleResult;
      record.remarks.push({
...
router.get('/classes/summary', (req, res) => {
  const role = getRole(req);
  res.json({ success: true, data: buildClassSummary(role) } as ApiResponse);
});

function buildClassSummary(role: UserRole) {
...
router.get('/babies/:id', (req, res) => {
  ...
  if (!baby) {
    return res.status(404).json({ success: false, error: '宝宝不存在' } as ApiResponse);
  }
  const babiesMap = new Map(db.babies.map(b => [b.id, b]));
...
router.get('/scans', (req, res) => {
  ...
  if (recordId) list = list.filter(s => s.recordId === recordId);

  const enriched: ScanRecord[] = list.map(s => {
...
router.get('/export', (req, res) => {
  ...
  const rows: any[] = db.records.map(r => {
...
router.get('/audit-logs', (req, res) => {
  ...
  if (role !== 'supervisor' && role !== 'admin') {
    return res.status(403).json({ success: false, error: '只有主管或管理员可以查看审计日志' } as ApiResponse);
  }
