...
import {
  Baby, DisinfectionRecord, ScanRecord, AuditLog, User,
  RecordStatus, UserRole, RemarkHistory
...
export class Database {
  ...
  addAudit(
    operatorId: string, operatorName: string, operatorRole: UserRole,
    action: string, recordId: string,
    oldStatus?: RecordStatus, newStatus?: RecordStatus,
  ...
  addRemark(recordId: string, remark: Omit<RemarkHistory, 'id'>) {
    const record = this.records.find(r => r.id === recordId);
    if (!record) return null;
    const fullRemark: RemarkHistory = { id: 'rm_' + uuidv4().slice(0, 8), ...remark };
  ...
  findBaby(id: string): Baby | undefined {
