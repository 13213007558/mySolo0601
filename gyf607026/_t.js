const fs=require('fs');
const p='/Users/guo/pro/solo/workspaces/gyf607026/public/index.html';
let s='';
const a=[
'<!DOCTYPE html>',
'<html lang="zh-CN">',
...
'async function loadStats(){try{const res=await fetch("/api/stats");const data=await res.json();const authorized=(data.byStatus.find(function(x){return x.auth_status==="已授权"})||{}).count||0;const unauthorized=(data.byStatus.find(function(x){return x.auth_status==="未授权"})||{}).count||0;const revoked=(data.byStatus.find(function(x){return x.auth_status==="授权撤回"})||{}).count||0;const manual=(data.bySource.find(function(x){return x.conclusion_source==="手工补录"})||{}).count||0;document.getElementById("statsGrid").innerHTML=\'<div class="stat-card"><div class="stat-label">总记录数</div><div class="stat-value">\'+data.total+\'</div></div>\'+\'<div class="stat-card green"><div class="stat-label">已授权</div><div class="stat-value">\'+authorized+\'</div></div>\'+\'<div class="stat-card red"><div class="stat-label">未授权</div><div class="stat-value">\'+unauthorized+\'</div></div>\'+\'<div class="stat-card orange"><div class="stat-label">授权撤回</div><div class="stat-value">\'+revoked+\'</div></div>\'+\'<div class="stat-card purple"><div class="stat-label">手工补录数</div><div class="stat-value">\'+manual+\'</div></div>\'+\'<div class="stat-card gray"><div class="stat-label">坏数据数</div><div class="stat-value">\'+data.badData+\'</div></div>\'+\'<div class="stat-card cyan"><div class="stat-label">历史版本总数</div><div class="stat-value">\'+data.totalVersions+\'</div></div>\'}catch(e){console.error("加载统计失败",e)}}',
];
s+=js.join('\n');
s+=`
async function loadRecords(){const phone=document.getElementById("phoneFilter").value.trim();const includeBad=document.getElementById("includeBad").checked?"1":"0";let url="/api/records?include_bad="+includeBad;if(phone)url+="&phone="+encodeURIComponent(phone);try{const res=await fetch(url);const records=await res.json();const tbody=document.getElementById("recordsTbody");if(records.length===0){tbody.innerHTML='<tr><td colspan="11" class="empty">暂无记录</td></tr>';return}tbody.innerHTML=records.map(function(r){return'<tr>'+'<td><strong>'+r.baby_id+'</strong> · '+r.baby_name+'</td>'+'<td>'+(r.parent_phone||'-')+'</td>'+'<td>'+(r.store_name||'-')+' / '+(r.class_name||'-')+'</td>'+'<td>'+(r.photo_type||'-')+'</td>'+'<td><span class="status-badge '+statusClass(r.auth_status)+'">'+(r.auth_status||'-')+'</span></td>'+'<td>'+(r.conclusion||'-')+'</td>'+'<td><span class="source-tag '+(r.conclusion_source==="手工补录"?"manual":"")+'">'+(r.conclusion_source||'-')+'</span></td>'+'<td><span class="version-tag">v'+r.version+'</span></td>'+'<td>'+(r.operator||'-')+'</td>'+'<td style="color:#64748b;font-size:12px;">'+r.updated_at+'</td>'+'<td><div class="cell-actions">'+'<button class="btn btn-sm btn-primary" onclick="viewDetail('+r.id+')">详情</button>'+'<button class="btn btn-sm btn-warning" onclick="openSupplement('+r.id+')">补录</button>'+'</div></td>'+'</tr>'}).join("")}catch(e){console.error("加载列表失败",e)}}
`;
s+=`
async function viewDetail(id){try{const res=await fetch("/api/records/"+id);const data=await res.json();const r=data.record;document.getElementById("detailTitle").textContent=r.baby_id+" · "+r.baby_name+" - 记录详情";let html="";if(data.accessReason){html+='<div class="alert-box"><strong>⚠️ 跨班/异常提示：</strong>'+data.accessReason+'</div>'}html+='<h3>📋 基本信息</h3><div class="info-grid">'+'<div class="info-item"><span class="info-label">宝宝ID</span><span class="info-value">'+r.baby_id+'</span></div>'+'<div class="info-item"><span class="info-label">宝宝姓名</span><span class="info-value">'+r.baby_name+'</span></div>'+'<div class="info-item"><span class="info-label">家长手机</span><span class="info-value">'+(r.parent_phone||'-')+'</span></div>'+'<div class="info-item"><span class="info-label">门店 / 班级</span><span class="info-value">'+(r.store_name||'-')+' / '+(r.class_name||'-')+'</span></div>'+'<div class="info-item"><span class="info-label">照片类型</span><span class="info-value">'+(r.photo_type||'-')+'</span></div>'+'<div class="info-item"><span class="info-label">当前版本</span><span class="info-value"><span class="version-tag">v'+r.version+'</span></span></div>'+'<div class="info-item"><span class="info-label">授权状态</span><span class="info-value"><span class="status-badge '+statusClass(r.auth_status)+'">'+(r.auth_status||'-')+'</span></span></div>'+'<div class="info-item"><span class="info-label">结论来源</span><span class="info-value"><span class="source-tag '+(r.conclusion_source==="手工补录"?"manual":"")+'">'+(r.conclusion_source||'-')+'</span></span></div>'+'<div class="info-item full"><span class="info-label">结论</span><span class="info-value">'+(r.conclusion||'-')+'</span></div>'+'<div class="info-item full"><span class="info-label">备注</span><span class="info-value">'+(r.remark||'-')+'</span></div>'+'<div class="info-item"><span class="info-label">操作人</span><span class="info-value">'+(r.operator||'-')+'</span></div>'+'<div class="info-item"><span class="info-label">更新时间</span><span class="info-value" style="font-size:13px;">'+r.updated_at+'</span></div>'+'</div>';if(data.sameBaby&&data.sameBaby.length>0){html+='<h3>👶 同宝宝其他记录 ('+data.sameBaby.length+')</h3>';data.sameBaby.forEach(function(s){html+='<div style="background:#f8fafc;border-radius:8px;padding:10px 14px;margin-bottom:8px;font-size:13px;">'+'<strong>'+s.baby_id+' · '+s.baby_name+'</strong>'+'<span style="margin:0 8px;color:#cbd5e1;">|</span>'+(s.store_name||'-')+' / '+(s.class_name||'-')+'<span style="margin:0 8px;color:#cbd5e1;">|</span><span class="status-badge '+statusClass(s.auth_status)+'">'+(s.auth_status||'-')+'</span>'+'<span style="margin:0 8px;color:#cbd5e1;">|</span><span class="source-tag '+(s.conclusion_source==="手工补录"?"manual":"")+'">'+(s.conclusion_source||'-')+'</span>'+'<span style="margin:0 8px;color:#cbd5e1;">|</span><span class="version-tag">v'+s.version+'</span>'+'<div style="margin-top:6px;color:#475569;">'+(s.conclusion||'')+'</div>'+'</div>'})}html+='<h3>🕐 历史版本时间线 ('+data.versions.length+')</h3>';if(data.versions.length===0){html+='<div class="empty" style="padding:24px;">暂无历史版本</div>'}else{html+='<div class="timeline">';data.versions.forEach(function(v){html+='<div class="timeline-item">'+'<div class="timeline-head">'+'<span class="timeline-version">v'+v.version+' · <span class="status-badge '+statusClass(v.auth_status)+'" style="font-size:11px;">'+(v.auth_status||'-')+'</span></span>'+'<span class="timeline-time">'+v.updated_at+'</span>'+'</div>'+'<div class="timeline-body">'+'<p><strong>结论：</strong>'+(v.conclusion||'-')+'</p>'+'<p><strong>操作人：</strong>'+(v.operator||'-')+'</p>'+(v.change_reason?'<p class="change-reason">💬 '+v.change_reason+'</p>':'')+'</div>'+'</div>'});html+='</div>'}html+='<h3>📝 操作日志 ('+data.logs.length+')</h3>';if(data.logs.length===0){html+='<div class="empty" style="padding:24px;">暂无操作日志</div>'}else{html+='<div class="log-list">';data.logs.forEach(function(l){html+='<div class="log-item">'+'<div class="log-head">'+'<span class="log-type">'+l.operation_type+'</span>'+'<span class="log-time">'+l.created_at+'</span>'+'</div>'+'<div class="log-detail">'+(l.detail||'')+'</div>'+'<div class="log-op">👤 '+(l.operator||'-')+' · '+(l.operator_role||'')+(l.reason?' · 原因: '+l.reason:'')+'</div>'+'</div>'});html+='</div>'}document.getElementById("detailBody").innerHTML=html;openModal("detailModal")}catch(e){console.error("加载详情失败",e)}}
`;
s+=`
...
async function submitCreate(){const baby_id=document.getElementById("createBabyId").value.trim();const baby_name=document.getElementById("createBabyName").value.trim();if(!baby_id||!baby_name){alert("宝宝ID和姓名为必填项");return}const payload={baby_id:baby_id,baby_name:baby_name,parent_phone:document.getElementById("createParentPhone").value.trim(),store_name:document.getElementById("createStoreName").value.trim(),class_name:document.getElementById("createClassName").value.trim(),photo_type:document.getElementById("createPhotoType").value,auth_status:document.getElementById("createAuthStatus").value,conclusion:document.getElementById("createConclusion").value.trim(),operator:document.getElementById("createOperator").value.trim(),remark:document.getElementById("createRemark").value.trim()};try{const res=await fetch("/api/records",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!res.ok){const err=await res.json();throw new Error(err.error||"创建失败")}closeModal("createModal");loadRecords();loadStats()}catch(e){alert("创建失败: "+e.message)}}
async function exportCSV(){const phone=document.getElementById("phoneFilter").value.trim();const includeBad=document.getElementById("includeBad").checked?"1":"0";let url="/api/export?include_bad="+includeBad;if(phone)url+="&phone="+encodeURIComponent(phone);window.location.href=url}
...
document.getElementById("phoneFilter").addEventListener("keydown",function(e){if(e.key==="Enter")loadRecords()});
