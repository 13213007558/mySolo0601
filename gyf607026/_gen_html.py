...
async function loadStats() {
  try {
    ...
    const authorized = (data.byStatus.find(function(x){return x.auth_status==='已授权'}) || {}).count || 0;
    const unauthorized = (data.byStatus.find(function(x){return x.auth_status==='未授权'}) || {}).count || 0;
    const revoked = (data.byStatus.find(function(x){return x.auth_status==='授权撤回'}) || {}).count || 0;
    const manual = (data.bySource.find(function(x){return x.conclusion_source==='手工补录'}) || {}).count || 0;
...
async function viewDetail(id) {
  try {
    ...
    html += '<h3>📋 基本信息</h3><div class="info-grid">'+
    ...
    if (data.sameBaby && data.sameBaby.length > 0) {
      ...
      data.sameBaby.forEach(function(s) {
        html += '<div style="background:#f8fafc;border-radius:8px;padding:10px 14px;margin-bottom:8px;font-size:13px;">'+
          ...
          '<span style="margin:0 8px;color:#cbd5e1;">|</span>'+(s.store_name||'-')+' / '+(s.class_name||'-')+
          '<span style="margin:0 8px;color:#cbd5e1;">|</span><span class="status-badge '+statusClass(s.auth_status)+'">'+(s.auth_status||'-')+'</span>'+
          '<span style="margin:0 8px;color:#cbd5e1;">|</span><span class="source-tag '+(s.conclusion_source==='手工补录'?'manual':'')+'">'+(s.conclusion_source||'-')+'</span>'+
          '<span style="margin:0 8px;color:#cbd5e1;">|</span><span class="version-tag">v'+s.version+'</span>'+
...
async function submitCreate() {
  ...
  if (!baby_id || !baby_name) { alert('宝宝ID和姓名为必填项'); return; }
  const payload = {
    baby_id: baby_id,
    baby_name: baby_name,
